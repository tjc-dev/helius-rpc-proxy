...
export default {
	async fetch(request: Request, env: Env) {
		...
		if (upgradeHeader || upgradeHeader === 'websocket') {
			return await fetch(`https://${env.HELIUS_ENV}.helius-rpc.com/?api-key=${env.HELIUS_API_KEY}`, request)
		}


		const { pathname, search } = new URL(request.url)
		const payload = await request.text();
		const proxyRequest = new Request(`https://${pathname === '/' ? `${env.HELIUS_ENV}.helius-rpc.com` : 'api.helius.xyz'}${pathname}?api-key=${env.HELIUS_API_KEY}${search ? `&${search.slice(1)}` : ''}`, {
